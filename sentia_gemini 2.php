<?php
// sentia_gemini_test.php — v3.0 GEMINI como motor principal (prompt completo)
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: https://neuroup.help');
header('Access-Control-Allow-Headers: Content-Type');
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
header('X-Sentia-Version: v3.0-gemini');
...
function logmsg($msg, $extra=null){
  ...
  @file_put_contents(__DIR__.'/sentia_gemini.log', $line.PHP_EOL, FILE_APPEND);
}
...
if ($_SERVER['REQUEST_METHOD']==='GET'){
  echo json_encode(['ok'=>true,'message'=>'pong','version'=>'v3.0-gemini']);
  exit;
}
...
// === SYSTEM PROMPT MAESTRO COMPLETO ===
$hora = (int)date('H');
...
MODULO AIDI — DATOS DEL DISPOSITIVO FISICO

Cuando AiDi te envie datos de telemetria, actua segun estos protocolos:

PROTOCOLO SILENCIO (Emergencia pasiva):
Si recibes [TOMA_OMITIDA: X minutos de retraso], actua asi:
- Retraso de 15 min: pregunta con calma "Hola, son las [hora] y aun no has tomado tu medicacion. Todo bien?"
- Retraso de 30 min: tono mas directo, pregunta si necesita ayuda
- Retraso de 60 min: alerta maxima, indica que se va a avisar a un familiar
Nunca uses un tono alarmante. La calma protege al paciente.

PROTOCOLO CONFUSION (Acceso fuera de hora):
Si recibes [ACCESO_FUERA_HORA], responde con empatia:
"Ya tomaste tu medicacion de [turno]. La proxima toma es a las [hora]. No te preocupes, es facil olvidarlo."
No desbloquees la noria. No reganes. Explica con calma.

PROTOCOLO TENDENCIA (Analisis de bienestar):
Si recibes [PATRON_MALESTAR: descripcion], genera internamente un resumen:
"El usuario ha reportado [sintoma] de forma repetida tras la toma de [turno]. Recomendacion: revision medica."
Comunicaselo al usuario con suavidad y sugiere que lo comente con su medico.

PROTOCOLO GUARDIAN (Silencio absoluto):
Si recibes [SIN_ACTIVIDAD: X horas], actua segun nivel:
- 2 horas fuera de hora de sueno: pregunta suave por voz
- 4 horas: alerta al contacto familiar configurado
- Emergencia confirmada: indica llamar al 112

Cuando no recibas datos de AiDi, funciona en modo Sentia estandar.
El bienestar emocional del usuario es siempre la prioridad.
TXT;

$system .= $memoryText;

// === RESUMEN VIVO DE SESIÓN (OpenAI mini) ===
$sessionSummary = '';
try {
    ...
    if($totalMensajes % 5 === 0) {
        ...
        if(!empty($conversationText)){
            $summaryPayload = [
                'model' => 'gpt-4o-mini',
                'messages' => [
                    ['role' => 'system', 'content' => 'Eres un analizador de conversaciones emocionales. Devuelve SOLO un JSON con esta estructura exacta, sin explicaciones ni markdown: {"persona":"descripcion breve","mensajes":numero,"temas":"temas mencionados","estado_emocional":"estado detectado","ultimo_tema":"de que se hablaba","tono_sentia":"como ha respondido Sentia"}'],
            ...
            if($summaryCode >= 200 && $summaryCode < 300){
                ...
                if(is_array($summaryData)){
                    $sessionSummary = "\n\nSESION ACTUAL:\n".
                        ...
                        "- Mensajes: ".($summaryData['mensajes'] ?? '?')."\n".
                        "- Temas: ".($summaryData['temas'] ?? 'ninguno')."\n".
                        "- Estado emocional: ".($summaryData['estado_emocional'] ?? 'neutro')."\n".
                        "- Ultimo tema: ".($summaryData['ultimo_tema'] ?? 'ninguno')."\n".
                        "- Tono Sentia: ".($summaryData['tono_sentia'] ?? 'neutro');
                }
            }
        }
    }
...
$systemConSesion = $system . $sessionSummary;
...
foreach($messages as $m){
    ...
    $geminiContents[] = ['role' => $geminiRole, 'parts' => [['text' => $content]]];
}

// === LLAMADA A GEMINI 2.5 FLASH ===
$geminiUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key='.$gemini_key;
$geminiBody = json_encode([
    'system_instruction' => ['parts' => [['text' => $systemConSesion]]],
...
$geminiResponse = curl_exec($chg);
$geminiErrno = curl_errno($chg);
$geminiError = curl_error($chg);
$geminiCode = curl_getinfo($chg, CURLINFO_HTTP_CODE);
curl_close($chg);

if($geminiErrno){ fail(500, 'cURL Gemini error: '.$geminiError.' ('.$geminiErrno.')'); }
if($geminiCode < 200 || $geminiCode >= 300){
    $parsedG = json_decode($geminiResponse, true);
    fail($geminiCode, 'HTTP '.$geminiCode.' desde Gemini', $parsedG ?: ['raw'=>$geminiResponse]);
}

$geminiData = json_decode($geminiResponse, true);
$reply = $geminiData['candidates'][0]['content']['parts'][0]['text'] ?? '';
if($reply === ''){ fail(500, 'Gemini devolvio respuesta vacia'); }

// === EXTRACTOR DE MEMORIA (OpenAI mini) ===
$memoryForClient = [];
$memoryCandidate = [];
...
try {
    ...
    if ($lastUser !== '') {
        $memoryPayload = [
            'model' => 'gpt-4o-mini',
            'messages' => [
                ['role' => 'system', 'content' => 'Eres un extractor unificado de memoria personal y comportamiento conversacional. Devuelve SOLO JSON valido con esta estructura exacta: {"memoria":{},"comportamiento":{}} MEMORIA — solo: familiares confirmados, mascotas con especie, ciudad/pais, trabajo, proyectos activos, gustos duraderos, aficiones, fechas futuras importantes. COMPORTAMIENTO — campos: preocupacion_recurrente, busca_validacion, aislamiento_social, tono_sesion, patron_observado. Si no hay evidencia clara devuelve objetos vacios. Sin explicaciones. Sin markdown. Solo JSON.'],
